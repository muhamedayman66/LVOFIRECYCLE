...
# تحسين معالجة الصور بشكل دقيق
def preprocess_image_pil(file_path):
    img_size = (224, 224)

    # فتح الصورة باستخدام PIL (يحافظ على الجودة الأصلية)
    img = Image.open(file_path)
    
    # التأكد من أن الصورة في الوضعية الصحيحة
    img = img.convert("RGB")  # تحويل للصيغة الصحيحة
    img = img.resize(img_size)  # تغيير الحجم فقط بدون أي تعديلات أخرى
    
    # تحويل الصورة إلى numpy array
    img_array = np.array(img) / 255.0  # Normalization إلى [0,1]
