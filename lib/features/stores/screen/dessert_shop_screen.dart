...
class DessertShopsScreen extends StatefulWidget {
  const DessertShopsScreen({super.key});

  @override
  _DessertShopsScreenState createState() => _DessertShopsScreenState();
}

class _DessertShopsScreenState extends State<DessertShopsScreen> {
  ...
  Future<void> fetchStores() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Map<String, dynamic>> dessertShops = [];
        for (var store in storesData) {
          if (store['category']?.toLowerCase() == 'dessert_shops') {
            String imagePath = "assets/images/desserts.jpg"; // Default image
            switch (store['name'].toLowerCase()) {
              case 'b.laban': // عدلناها من 'blab' لـ 'b.laban'
                imagePath = "assets/images/BlabnDS.jpg";
                break;
              case 'la poire': // عدلناها من 'lapoire' لـ 'la poire'
                imagePath = "assets/images/LapoireDS.jpg";
                break;
            }
            dessertShops.add({
              "name": store['name'],
        ...
        setState(() {
          allStores = dessertShops;
          filteredStores = dessertShops;
          isLoading = false;
        });

        if (dessertShops.isEmpty) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('No dessert shops available at the moment'),
              ),
            );
          }
        }
