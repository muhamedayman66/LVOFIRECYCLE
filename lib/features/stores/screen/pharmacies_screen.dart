    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var store in storesData) {
          if (store['category']?.toLowerCase() == 'pharmacies') {
            String imagePath = "assets/images/pharmacies.jpg"; // Default image
            switch (store['name'].toLowerCase()) {
              case 'misr pharmacies':
                imagePath = "assets/images/MisrPh.jpg";
                break;
              case 'el ezaby': // عدلناها من 'ezaby pharmacies' لـ 'el ezaby'
                imagePath = "assets/images/EzabyPh.jpg";
                break;
            }
            pharmacies.add({
              "name": store['name'],
        ...
        if (pharmacies.isEmpty) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('No pharmacies available at the moment'),
              ),
            );
          }
        }
